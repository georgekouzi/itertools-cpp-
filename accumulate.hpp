...
namespace itertools
{
    ...
    public:
        ...
        class iterator
        {
            typename decay<decltype(*(container.begin()))>::type resulte;
        ...
        public:
            iterator(decltype((container.begin())) it, decltype((container.begin())) ite, function f) : first_index(it), last_index(ite), _f(f)
            {
                if (it != ite)
                    resulte = *it;
            };
